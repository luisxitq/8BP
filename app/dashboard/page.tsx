...
export default function DashboardPage() {
  ...
  function LicenseCard({ lic }: { lic: License }) {
    ...
    return (
      <div className="rounded-2xl ie-card p-4 sm:p-5 ie-anim-in">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    // Fallback: PATCH remove_device on main license route
                    if (!res.ok) {
                      res = await fetch(`/api/licenses/${encodeURIComponent(lic.id)}`, {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ remove_device: d }),
                      });
                      data = await res.json().catch(() => ({}));
                    }
                    if (!res.ok) throw new Error(data.error || res.statusText || 'Error al eliminar HWID');
                    await load();
                  } catch (e) {
                    alert(String(e));
                  } finally {
                ...
                className="ie-chip inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-zinc-800/80 border border-zinc-700/80 text-[11px] font-mono text-sky-400 hover:border-red-500/50 hover:text-red-300 touch-manipulation"
              >
  ...
  return (
    <div className="min-h-[100dvh] pb-8">
      <header className="border-b border-red-950/60 bg-black/70 backdrop-blur sticky top-0 z-20">
        <div className="max-w-3xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="font-semibold leading-tight text-sm sm:text-base truncate">
                <span className="text-white">Itachi</span><span className="text-red-500">Engine</span>
              </h1>
              <p className="text-[10px] sm:text-xs text-zinc-500 hidden sm:block">License Panel</p>
            </div>
          </div>

          <div className="hidden sm:flex items-center gap-2">
            <button onClick={() => setShowCreate(true)} className="px-3 py-1.5 rounded-xl ie-btn-primary text-sm transition touch-manipulation">
              + Create Key
            </button>
          ...
          <div className="flex sm:hidden items-center gap-1.5">
            <button onClick={() => setShowCreate(true)} className="px-2.5 py-1.5 rounded-xl ie-btn-primary text-xs touch-manipulation">
              + Key
            </button>
        ...
        {menuOpen && (
          <div className="sm:hidden border-t border-red-950/50 px-3 py-2 flex gap-2 bg-black/80">
      ...
      <main className="max-w-3xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-5 ie-stagger">
          {[
            ...
            <div key={s.label} className="rounded-xl ie-card ie-stat px-3 py-2.5 sm:px-4 sm:py-3">
        ...
        {loading ? (
          ...
          <div className="text-center py-14 border border-dashed border-zinc-800 rounded-2xl px-4">
            ...
            <button onClick={() => setShowCreate(true)} className="px-4 py-2.5 rounded-xl ie-btn-primary text-sm touch-manipulation">
              Crear primera key
            </button>
          </div>
        ) : (
          <div className="space-y-3 sm:space-y-4 ie-stagger">
            {licenses.map((lic) => (
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/75 ie-anim-fade">
          <div className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl border border-zinc-700 bg-[#161618] p-5 sm:p-6 shadow-2xl max-h-[92dvh] overflow-y-auto ie-anim-modal">
